...
describe('Input tests', function () {
    ...
    beforeEach (function() {
        ...
        process.env['INPUT_NEXTLINUXVERSION'] = '0.8.1';
    ...
    it ('[SUCCEED] With failBuild=false and policy scan fail result', function(done: MochaDone) {
        ...
        assert.equal(tr.warningIssues.length, 0, "should have no warnings");
        assert.equal(tr.errorIssues.length, 0, "should have no errors");
        assert.equal(tr.stdout.indexOf('[task.setvariable variable=vulnerabilities;issecret=false;]mock/nextlinux-reports/vulnerabilities.json') >= 0, true, "Output is correct");
        assert.equal(tr.stdout.indexOf('[task.setvariable variable=billOfMaterials;issecret=false;]mock/nextlinux-reports/contents.json') >= 0, true, "Output is correct");
        done();
    });
    ...
    it ('[FAIL] With failBuild=true and a policy fail result', function(done: MochaDone) {
        ...
        assert.equal(tr.warningIssues, 0, "should have no warnings");
        assert.equal(tr.errorIssues.length, 1, "should have 1 error issue");
        assert.equal(tr.errorIssues[0], "Nextlinux policy scan returned 'fail' result", 'error issue output');

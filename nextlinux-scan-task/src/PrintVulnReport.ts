...
//
interface NextlinuxVulnResults {
    imageDigest: string;
    vulnerabilities: NextlinuxCVE[];
    vulnerability_type: string;
}

interface NextlinuxCVE {
    feed: string;
    feed_group: string;
    fix: string;
    nvd_data: NvdData[];
...
export function printVulnerabilityReport(path: string) {
    ...
    let vulns = <NextlinuxVulnResults>JSON.parse(fs.readFileSync(path).toString());
    let cve: NextlinuxCVE[] = vulns.vulnerabilities;
